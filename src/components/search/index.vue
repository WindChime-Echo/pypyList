    ...
    <el-form :inline="true" @submit.native.prevent>
      <el-form-item>
        <el-input
          ...
          :placeholder="`请输入${searchType === '0' ? 'ID' : '曲名'}`"
        >
          <template #prepend>
...
const search = () => {
  ...
  const originData = !typeKey.value
    ? props.rawData
    : props.rawData.filter((item) => item.group === typeKey.value)
  if (!keyword.value?.trim()) return emit("update:modelValue", originData)

  switch (String(searchType.value)) {
    case "0":
      const numReg = /^[1-9]\d*$/
      if (!numReg.test(keyword.value))
        return ElMessage.error("请输入数字类型ID")
      const index = originData.findIndex(
        (item) => String(item.id) === keyword.value
      )
